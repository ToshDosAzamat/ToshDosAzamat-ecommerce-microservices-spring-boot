package com.example.authorserver.config;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests((authorize) ->
                        authorize
                                .requestMatchers("/api/user/**","/api/client/**").permitAll()
    ...
    public RegisteredClientRepository registeredClientRepository() {
        List<RegisteredClient> registrationList = new ArrayList<>();
        ...
        RegisteredClient orderClient = RegisteredClient.withId(UUID.randomUUID().toString())
                .clientId("order")
                .clientSecret(passwordEncoder().encode("order"))
                .redirectUri("https://oidcdebugger.com/debug")
                .scopes(s -> {
                    s.add(OidcScopes.OPENID);
                    s.add(OidcScopes.PROFILE);
        ...
        RegisteredClient paymentClient = RegisteredClient.withId(UUID.randomUUID().toString())
                .clientId("payment")
                .clientSecret(passwordEncoder().encode("payment"))
                .redirectUri("https://oidcdebugger.com/debug")
                .scopes(s -> {
                    s.add(OidcScopes.OPENID);
                    s.add(OidcScopes.PROFILE);
                ...
                .clientSettings(ClientSettings.builder().requireAuthorizationConsent(true).build())
                .build();
        registrationList.add(productClient);
        registrationList.add(orderClient);
        registrationList.add(paymentClient);
        return new InMemoryRegisteredClientRepository(registrationList);
    }
